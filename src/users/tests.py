...
#         self.verify_email_url = reverse('verify_email', kwargs={'auth_token': 'dummy_token'})  # 이메일 인증 토큰
#
#         # 테스트 유저 정보
...
#             is_active=False
#         )
#         auth_token = Token.generate_token(user)
#         email_verification_token = auth_token.refresh
#
#         # 이메일 인증 호출
#         verification_url = reverse('verify_email', kwargs={'auth_token': email_verification_token})
...
#         refresh_token = login_response.cookies['refresh'].value
#
#         # refresh token을 사용해 access auth_token 재발급
